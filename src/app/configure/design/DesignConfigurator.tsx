      ...
      <div ref={containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-zinc-400 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
        ...
        >
          <div className='relative size-full'>
            <NextImage src={imageUrl} fill alt='user custom image' className='pointer-events-none' />
          </div>
        </Rnd>
      </div>

      <div className='h-[37.5rem] w-full col-span-full lg:col-span-1 flex flex-col bg-white border-2'>
              ...
              <div className='flex flex-col gap-6'>
                <RadioGroup value={options.color} onChange={(val) => {
                  ...
                  <div className='mt-3 flex items-center space-x-3'>
                    {COLORS.map((color) => (
                      <RadioGroup.Option key={color.label} value={color} className={({ active, checked }) => cn("relative -m-0.5 flex cursor-pointer items-center justify-center rounded-full p-1 active:ring-0 focus:ring-0 active:outline-none focus:outline-none border-[3px] border-transparent", {
                        [`border-${color.tw}`]: active || checked,
                      })}>
                        <span className={cn(`bg-${color.tw}`, "h-8 w-8 rounded-full")} />
                      </RadioGroup.Option>
                    ))}
                  </div>
                </RadioGroup>
