...
const Page = (props: Props) => {
  ...
  const { startUpload, isUploading } = useUploadThing("imageUploader", {
    onClientUploadComplete: ([data]) => {
      ...
      startTransition(() => {
        // giving our design page a searchParam of "id" which nextjs will pass to the next route
        ...
        >
          {({ getRootProps, getInputProps }) => (
            <div className='size-full flex-1 flex flex-col items-center justify-center' {...getRootProps()}>
              <input {...getInputProps()} />
              {isDragOver ? (<MousePointerSquareDashed className='size-6 text-zinc-500 mb-2' />) : (isUploading || isPending) ? (<Loader2 className='animate-spin size-6 text-zinc-500 mb-2' />) : (<Image className='size-6 text-zinc-500 mb-2' />)}
